...
public class BasicParticle {
	...
	public static enum ShapeType {
		CIRCLE, HEAD, TORSO, ARM, LEG
	}

	public BasicParticle(double sx, double sy, double vx, double vy, double radius, boolean improvedEuler, Color col, double mass, double dragForce, ShapeType shapeType) {
		...
		bodyDef.position.set((float)sx,(float)sy);
		bodyDef.linearVelocity.set((float)vx, (float)vy);
	...
	public void draw(Graphics2D g) {
		...
		switch (shapeType) {
			...
			case HEAD:
				...
				break;
			case TORSO:
				g.fillRect(x - SCREEN_RADIUS, y - SCREEN_RADIUS, 2 * SCREEN_RADIUS, 3 * SCREEN_RADIUS);
				break;
			case ARM:
			case LEG:
				g.fillRect(x - SCREEN_RADIUS, y - SCREEN_RADIUS, 2 * SCREEN_RADIUS, 5 * SCREEN_RADIUS);
				break;
		}
	}
	...
	public void notificationOfNewTimestep() {
		if (0.02f>0) {
			...
			dragForce1=dragForce1.mul((float) (-0.02f*mass));
			body.applyForceToCenter(dragForce1);
		}
		Vec2 velocity = body.getLinearVelocity();
	    float speed = velocity.length();
	    rotation += speed / SCREEN_RADIUS;
	}
}
